setwd("/Users/liuzhe/Desktop/CTC/CTC_Prognosis_COAD/07_ChemotherapyResistanceAnalysis/oncoPredict_analysis/drugs")
rm(list = ls())

risk<-read.table("/Users/liuzhe/Desktop/CTC/CTC_Prognosis_COAD/02_ConstructionRiskModel/04_multivariate_Cox_regression_analysis/risk.txt",
                 header=T,row.names = 1)
risk$sample_name<-row.names(risk)
risk$sample_name<-substr(risk$sample_name,1,12)
dim(risk)
#[1] 521   9
ic50<-read.csv("/Users/liuzhe/Desktop/CTC/CTC_Prognosis_COAD/07_ChemotherapyResistanceAnalysis/oncoPredict_analysis/calcPhenotype_Output/DrugPredictions.csv")
ic50$sample_name<-ic50$X
ic50$sample_name<-substr(ic50$sample_name,1,12)
dim(ic50)
#[1] 449  200
...
#merged_rmNA<-na.omit(merged)
#dim(merged_rmNA)
#[1] 135  63
for(i in 1:nrow(merged)){
  merged$X[i]<-paste(merged$X[i],i,sep="_")
}
rownames(merged)<-merged$X
merged_format<-cbind(merged[,ncol(merged)],merged[,3:(ncol(merged)-2)])
####################################drug1： WIKI4_1940################################################
drug_sen_WIKI4_1940<-cbind(merged_format$`merged[, ncol(merged)]`,as.numeric(merged_format$WIKI4_1940))
rownames(drug_sen_WIKI4_1940)<-row.names(merged_format)
colnames(drug_sen_WIKI4_1940)<-c("Risk","IC50")
head(drug_sen_WIKI4_1940)
drug_sen_WIKI4_1940<-as.data.frame(drug_sen_WIKI4_1940)
drug_sen_WIKI4_1940$IC50<-as.numeric(drug_sen_WIKI4_1940$IC50)
library(ggpubr)
pdf("WIKI4_1940.pdf")
p <- ggboxplot(drug_sen_WIKI4_1940, x="Risk",
               y = "IC50", color = "Risk",
...
####################################drug2： Cisplatin_1005 ################################################
drug_sen_Cisplatin_1005 <-cbind(merged_format$`merged[, ncol(merged)]`,as.numeric(merged_format$Cisplatin_1005))
rownames(drug_sen_Cisplatin_1005)<-row.names(merged_format)
colnames(drug_sen_Cisplatin_1005)<-c("Risk","IC50")
head(drug_sen_Cisplatin_1005)
drug_sen_Cisplatin_1005 <-as.data.frame(drug_sen_Cisplatin_1005)
drug_sen_Cisplatin_1005$IC50<-as.numeric(drug_sen_Cisplatin_1005$IC50)
pdf("Cisplatin_1005.pdf")
p <- ggboxplot(drug_sen_Cisplatin_1005 , x="Risk",
               y = "IC50", color = "Risk",
               palette = "jco", add = "jitter")
# 添加p值
p + stat_compare_means()
...
setwd("D:/MyProjects/CTC/CTC_Prognosis_COAD/07_ChemotherapyResistanceAnalysis/TCGA/drugs/Figures/")
