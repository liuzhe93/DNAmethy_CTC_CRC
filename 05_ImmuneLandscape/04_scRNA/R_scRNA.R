setwd("D:/MyProjects/CTC/CTC_Prognosis_COAD/05_ImmuneLandscape/04_scRNA")
rm(list = ls()) 

library("ggplot2")
dat<-read.table("CRC_GSE146771_Smartseq2_CellMetainfo_table.tsv", sep = "\t", header = T)
#############################malignancy##################################################
...
dat<-Read10X_h5("CRC_GSE146771_Smartseq2_expression.h5")
dim(dat)
#[1] 15178 10468
...
celltype_data<-read.table("CRC_GSE146771_Smartseq2_CellMetainfo_table.tsv", sep = "\t", header = T)
dim(RFS)
#[1] 10468     2
dim(celltype_data)
#[1] 10468    13
...
#  B    CD4Tconv        CD8T      CD8Tex Endothelial Fibroblasts   Malignant        Mast  Mono/Macro          NK      Plasma 
#324        1708         591         788          78         139        1057         260        1498        2116         611 
#Tprolif        Treg 
#236        1062
...
GLMS_CD8T_CD8Tg<-subset(GLMS_data,group!="CD8T")
GLMS_CD8T_avg_rest<-mean(GLMS_CD8T_CD8Tg$RiskFactorScore)
log2_CD8T<-log2(GLMS_CD8T_avg/GLMS_CD8T_avg_rest)

GLMS_CD8Tex<-subset(GLMS_data,group=="CD8Tex")
GLMS_CD8Tex_avg<-mean(GLMS_CD8Tex$RiskFactorScore)
GLMS_CD8Tex_bg<-subset(GLMS_data,group!="CD8Tex")
GLMS_CD8Tex_avg_rest<-mean(GLMS_CD8Tex_bg$RiskFactorScore)
log2_CD8Tex<-log2(GLMS_CD8Tex_avg/GLMS_CD8Tex_avg_rest)
...
GLMS_NK<-subset(GLMS_data,group=="NK")
GLMS_NK_avg<-mean(GLMS_NK$RiskFactorScore)
GLMS_NK_bg<-subset(GLMS_data,group!="NK")
GLMS_NK_avg_rest<-mean(GLMS_NK_bg$RiskFactorScore)
log2_NK<-log2(GLMS_NK_avg/GLMS_NK_avg_rest)
...
GLMS_Treg<-subset(GLMS_data,group=="Treg")
GLMS_Treg_avg<-mean(GLMS_Treg$RiskFactorScore)
GLMS_Treg_bg<-subset(GLMS_data,group!="Treg")
GLMS_Treg_avg_rest<-mean(GLMS_Treg_bg$RiskFactorScore)
log2_Treg<-log2(GLMS_Treg_avg/GLMS_Treg_avg_rest)

data.matrix<-matrix(c(log2_B,"B",log2_CD4Tconv,"CD4Tconv",log2_CD8T,"CD8T",log2_CD8Tex,"CD8Tex",log2_Endothelial,"Endothelial",
                      log2_Fibroblasts,"Fibroblasts",log2_Malignant,"Malignant",log2_Mast,"Mast",log2_MonoMacro,"Mono/Macro",log2_NK,"NK",
                      log2_Plasma,"Plasma",log2_Tprolif,"Tprolif",log2_Treg,"Treg"),c(13,2),byrow = T)
